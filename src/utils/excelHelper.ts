...
export function detectMaintenanceFrequency(
  ...
  // 0. Explicit prefix pattern in text or description: e.g. "TH;3M; ...", "TH;1M;", "TH;6M;", "TH;4M;", "TH;2Y;", "TH;1Y;"
  const thMatch =
    upperCombined.match(/(?:^|[\s;,:])TH\s*[:;]\s*(\d+[MYW])\b/i) ||
    upperCombined.match(/(?:^|[\s;,:])TH\s*[:;]\s*(\d+[MYW])\s*;/i) ||
    upperCombined.match(/(?:^|[\s;,:])(\d+[MYW])\s*;\s*[A-Z]/i);
  if (thMatch) {
    const code = thMatch[1].toUpperCase();
...
/**
 * Splits a concatenated description string containing multiple maintenance tasks or equipment
 * (e.g. "TH;3M; Plate Heat Exchanger & TH;3M; Plate Heat Exchanger") into individual task items.
 */
export function splitMultipleWorkDescriptions(descVal: string): string[] {
  if (!descVal) return [];
  const trimmed = descVal.trim();
  if (!trimmed) return [];

  // Protect equipment with literal '&' in their standard names
  if (
    /^F\s*&\s*E\s*Tank$/i.test(trimmed) ||
    /differential\s*by-?pass\s*valve\s*&\s*control/i.test(trimmed) ||
    /chemical\s*dosing\s*pump\s*\/\s*tank/i.test(trimmed) ||
    /panel\s*&\s*filter/i.test(trimmed)
  ) {
    return [trimmed];
  }
  ...
  if (trimmed.includes('&')) {
    ...
    if (parts.length > 1) {
      // Check if parts look like distinct tasks or equipment:
      // e.g. multiple "TH;..." prefixes, or duplicate equipment names, or distinct equipment names
      const hasMultipleTH = parts.filter((p) => /TH\s*[:;]|\b\d+[MYW]\b/i.test(p)).length >= 2;
      const isDuplicateEquip = parts[0].toLowerCase() === parts[1].toLowerCase();
      const hasEquipmentWords = parts.filter((p) =>
        /exchanger|chiller|pump|unit|filter|damper|screen|fan|tank|valve|strainer|ahu|fcu|pau|pwr|phe|heater|motor|compressor|cooler/i.test(p)
      ).length >= 2;

      if (hasMultipleTH || isDuplicateEquip || hasEquipmentWords) {
        return parts;
      }
    }
  }

  // Check if joined by newline
  if (trimmed.includes('\n')) {
...
export function findMatchingStandardItem(
  ...
  if (isPheQuery) {
    ...
    const b2Match = combinedText.match(/B2[/-](\d+)/i) || combinedText.match(/PHE[/-]?B2[/-](\d+)/i);
    if (b2Match) {
    ...
    // 2. Check for unit number in ASSETNUM or combinedText: e.g. PHE-001, PHE-002, PHE-1, PHE002, PHE 002
    // Specifically matches PHE followed by unit number or -001/-002 when asset contains PHE
    const pheNumMatch =
  ...
  // C. Group 1 / Group 2 / G01 / G02 matching
  ...
  // D. Specific numbered items in template: (NO:1), (NO:2), (01), (02)
  for (let i = 0; i < template.items.length; i++) {
    const std = template.items[i];
    ...
    // Numbered specific item like (NO:1), (NO:2), (01), (02), -001, -002
    // Must use token-bounded checks so digits in dates or work orders don't falsely trigger
    const noMatch = stdDescUpper.match(/(?:NO:?|NO\.?|-|\()\s*0*(\d+)\)?/i);
    if (noMatch) {
      const itemNum = parseInt(noMatch[1], 10);
      const mainKeyword = stdDescUpper.replace(/\([^)]+\)/g, '').replace(/[-_]\d+/g, '').trim();
      const cleanKeyword = cleanStr(mainKeyword);
      const numPattern = new RegExp(`(?:[-_\\s(:]|NO:?|NO\\.?)0*${itemNum}(?:[-_\\s):]|$)`, 'i');
      if (
        cleanKeyword.length >= 4 &&
        cleanedCombined.includes(cleanKeyword) &&
        numPattern.test(combinedText)
      ) {
        return { matchedIndex: i, matchedItem: std };
      }
    }
...
export function parseGenericTableRows(
  ...
  for (let r = headerRowIndex + 1; r < jsonRows.length; r++) {
    ...
    // Check if the description cell contains multiple concatenated items
    // (e.g. "TH;3M; Plate Heat Exchanger & TH;3M; Plate Heat Exchanger")
    const descParts = splitMultipleWorkDescriptions(descVal);
    const woParts = woNumVal ? woNumVal.split(/\s*[&,\n/]\s*|\s{2,}/).map((w) => w.trim()).filter(Boolean) : [];
    const assetParts = assetNumVal ? assetNumVal.split(/\s*[&,\n/]\s*|\s{2,}/).map((a) => a.trim()).filter(Boolean) : [];
    const jpNumParts = jpNumVal ? jpNumVal.split(/\s*[&,\n/]\s*|\s{2,}/).map((j) => j.trim()).filter(Boolean) : [];

    const numSubItems = Math.max(descParts.length, 1);
    for (let pIdx = 0; pIdx < numSubItems; pIdx++) {
      const curDescVal = descParts[pIdx] || descVal;
      const curWoNumVal = woParts[pIdx] || (woParts.length === 1 ? woParts[0] : '') || woNumVal || assetNumVal;
      let curAssetNumVal = assetParts[pIdx] || (assetParts.length === 1 ? assetParts[0] : '') || assetNumVal;

      // If multiple concatenated PHE items in a single cell without separate asset numbers, synthesize unit sequence
      if (descParts.length > 1 && !/0*([1-9]\d*)\b/.test(curAssetNumVal) && /PHE|PLATE\s*HEAT/i.test(curDescVal)) {
        curAssetNumVal = `${curAssetNumVal} PHE-00${pIdx + 1}`;
      }

      const curJpNumVal = jpNumParts[pIdx] || jpNumVal;

      // Detect station code for this sub-item:
      // USER RULE: First check ASSETNUM first 3 letters (e.g. LAK from LAK-ECS-ACC-101, TIC from TIC-ECS-PHE-001)
      const rowStn = (
        detectLocationCode(curAssetNumVal) ||
        detectLocationCode(locVal) ||
        detectLocationCode(curJpNumVal) ||
        detectLocationCode(assetDescVal) ||
        detectLocationCode(curDescVal) ||
        ''
      ).toUpperCase();
      ...
      const wonum = curWoNumVal || curAssetNumVal;
      ...
      let workDescription = '';
      if (assetDescVal && descParts.length <= 1) {
        workDescription = extractMeaningfulAssetWork(assetDescVal);
      }

      if (!workDescription && (curDescVal || generalDescVal || workOrderDescVal)) {
        const stdDesc =
          matchStandardWorkDescription(curDescVal) ||
          matchStandardWorkDescription(generalDescVal) ||
          matchStandardWorkDescription(workOrderDescVal);
        workDescription = stdDesc || cleanWorkDescription(curDescVal || generalDescVal || workOrderDescVal);
      }

      if (!workDescription && curAssetNumVal) {
        const std = matchStandardWorkDescription(curAssetNumVal);
        if (std) workDescription = std;
      }

      // Never set 'Air Cooled Chiller' as fallback default answer!
      if (!workDescription) {
        const fallback = cleanWorkDescription(curDescVal || generalDescVal || workOrderDescVal || assetDescVal || wonum);
      ...
      const detectedFreq = detectMaintenanceFrequency(
        `${curDescVal} ${generalDescVal} ${workOrderDescVal} ${assetDescVal}`,
        jobPlanDescVal,
        curJpNumVal
      );
      ...
      if (matchResult && matchResult.matchedIndex < stnItems.length) {
        // If same, record the WONUM in PM W/O, if they have more than one WONUM, each WONUM one line
        ...
        // Set frequency
        if (detectedFreq.m3) target.m3 = '100%';
        else if (detectedFreq.m4) target.m4 = '100%';
        ...
        else if (detectedFreq.m) target.m = '100%';
        else target.m = '100%';
      } else {
      ...
      // Summary tracking
      const sumKey = matchResult ? matchResult.matchedItem.workDescription : workDescription;
      ...
      const sumEntry = summaryMap.get(sumKey)!;
      sumEntry.count++;
      if (wonum && !sumEntry.wos.includes(wonum)) {
        sumEntry.wos.push(wonum);
      }
    }
  }
